...
public class BookReader {
    ...
    public static void main(String[] args) {
         ...
         */
            ...
            int lineCount = 0;
            boolean inBookText = false; // Are you reading between the start and end of markers?
            try (Scanner fileInput = new Scanner(bookFile)){
                //Loop until the end of file is reached
                while (fileInput.hasNextLine()){
                    //read the next line into 'line of text'
                    ...
                    if (lineOfText.startsWith(END_MARKER)){
                        break; // once the program find the end, break out of the loop
                    }
                    if (inBookText) {
                        //increment the line count
                        lineCount++;
                        //print line count and line of text
